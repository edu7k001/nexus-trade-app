...
app.post('/api/cashinpay/deposit', async (req, res) => {
    ...
    try {
        // 1. Chama a API da CashinPay para criar a transação
        ...
        // 2. Extrai o texto PIX (copia e cola) da resposta
        // A CashinPay retorna pix.qrcode (texto) e pix.copy_paste (mesmo texto)
        const pixText = transaction.pix.copy_paste || transaction.pix.qrcode;

        if (!pixText) {
            throw new Error('Resposta da CashinPay não contém o código PIX');
        }

        // 3. Gera a imagem do QR Code a partir do texto
        ...
        res.json({
            success: true,
            qrCodeImage: qrCodeImage,   // imagem base64 pronta para usar no src
            copyPaste: pixText,          // código para copiar
