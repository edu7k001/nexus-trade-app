    ...
    db.run(`CREATE TABLE IF NOT EXISTS admin_config (
        ...
        if (!err) {
            db.get('SELECT * FROM admin_config WHERE id = 1', (err, row) => {
                if (!row) {
                    db.run(`INSERT INTO admin_config (id, pix_key, min_deposit, bonus_amount, min_withdraw) 
...
// ===== CONFIRMAÇÃO DE DEPÓSITO (CORRIGIDA E ROBUSTA) =====
                ...
                db.run('UPDATE users SET balance = ?, total_deposits = total_deposits + ?, status = "Ativo" WHERE id = ?',
                    [balanceAfter, valor, deposit.user_id], function (err) {
                        ...
                        db.run(`INSERT INTO transactions (user_id, type, amount, balance_before, balance_after, reference_id, description)
                            ...
                            (err) => {
                                if (err) console.error('Erro ao registrar transação:', err); // Não crítico
                            });
