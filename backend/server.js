...
// Armazenar conexões WebSocket (userId -> WebSocket)
...
// Banco de dados SQLite (com serialize para garantir ordem)
...
app.post('/api/register', async (req, res) => {
    ...
    try {
        ...
        db.run(`INSERT INTO users (name, email, password, pix_key, cpf, phone) VALUES (?, ?, ?, ?, ?, ?)`,
            [name, email, hashedPassword, pixKey || '', cpf || '', phone || ''],
            function (err) {
                if (err) {
...
// ===== ROTA DE ATUALIZAÇÃO DE USUÁRIO (CORRIGIDA E COM LOGS) =====
app.post('/api/admin/user/:id/update', checkAdmin, (req, res) => {
    console.log('🚀 Rota de update foi chamada!');
    const { id } = req.params;
    const updates = req.body;
    console.log('📥 Dados recebidos:', { id, updates });

    // Lista de colunas permitidas para atualização
    const allowedFields = ['name', 'email', 'cpf', 'phone', 'pix_key', 'balance', 'status', 'rtp_individual'];
    ...
    if (fields.length === 0) {
        console.log('❌ Nenhum campo válido enviado');
    ...
    db.run(sql, values, function(err) {
        ...
        if (this.changes === 0) {
            console.log('❌ Usuário não encontrado (ID inválido)');
            return res.status(404).json({ error: 'Usuário não encontrado' });
        }

        console.log('✅ Usuário atualizado com sucesso. Linhas afetadas:', this.changes);
...
app.post('/api/admin/confirm-deposit/:id', checkAdmin, (req, res) => {
    ...
    if (!id) return res.status(400).json({ error: 'ID do depósito não fornecido' });
    ...
    db.serialize(() => {
        ...
        db.get('SELECT user_id, status FROM deposits WHERE id = ?', [id], (err, deposit) => {
            if (err) {
                db.run('ROLLBACK');
                console.error('Erro ao buscar depósito:', err);
            ...
            db.get('SELECT balance FROM users WHERE id = ?', [deposit.user_id], (err, user) => {
                ...
                db.run('UPDATE users SET balance = ?, total_deposits = total_deposits + ?, status = "Ativo" WHERE id = ?',
                    [balanceAfter, valor, deposit.user_id], function (err) {
                        ...
                        db.run(`INSERT INTO transactions (user_id, type, amount, balance_before, balance_after, reference_id, description)
                                VALUES (?, ?, ?, ?, ?, ?, ?)`,
                            [deposit.user_id, 'deposit', valor, balanceBefore, balanceAfter, id, 'Depósito confirmado'],
                            (err) => {
                        ...
                        db.run('COMMIT', (err) => {
                            ...
                            sendRealTimeUpdate(deposit.user_id, 'deposit_confirmed', {
                                amount: valor, newBalance: balanceAfter
                            });
                            sendToAllAdmins('deposit_processed', { id, userId: deposit.user_id, amount: valor });
                    ...
                    [balanceAfter, withdraw.amount, withdraw.user_id], function (err) {
                        ...
                        db.run('COMMIT', (err) => {
                            ...
                            sendRealTimeUpdate(withdraw.user_id, 'withdraw_approved', {
                                amount: withdraw.amount, newBalance: balanceAfter
                            });
    ...
    db.run('UPDATE withdraw_requests SET status = "Rejeitado", processed_by = ?, notes = ? WHERE id = ?',
        [req.admin.id, reason || '', id], function (err) {
            ...
            db.get('SELECT user_id, amount FROM withdraw_requests WHERE id = ?', [id], (err, withdraw) => {
                if (withdraw) {
                    sendRealTimeUpdate(withdraw.user_id, 'withdraw_rejected', { amount: withdraw.amount, reason });
                }
            });
